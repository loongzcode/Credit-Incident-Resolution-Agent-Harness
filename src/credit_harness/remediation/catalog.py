...
from .models import (
    ...
    EvidenceCondition as C, GapRequirement as G, RejectReason as R, RemediationActionContract,
)


def contract(action, risk, description, conditions, scope):
    l2 = risk == L.L2_SINGLE_ORDER_SIDE_EFFECT
    return RemediationActionContract(action_type=action, risk_level=risk, description=description,
...
class RemediationActionCatalog:
    entries = (
        contract(A.NO_REMEDIATION, L.L0_READ_ONLY, "保持当前业务状态，仅记录本轮不建议修复。", (), E.NONE),
        contract(A.REQUEST_OPERATOR_REVIEW, L.L1_ADMINISTRATIVE, "建议人工获取缺失事实或检查被阻断问题。", (C.OPEN_PROBLEM,), E.RECONCILIATION),
        contract(A.CREATE_RECONCILIATION_TASK, L.L1_ADMINISTRATIVE, "建议将已观察到的未收敛送入对账队列；本阶段不创建任务。", (C.OBSERVED_DIVERGENCE,), E.RECONCILIATION),
        contract(A.REPLAY_CALLBACK_CONSUMPTION, L.L2_SINGLE_ORDER_SIDE_EFFECT, "建议重放一个已验签且关联明确的失败消息；不是重新放款。",
                 (C.PAYMENT_IDENTITY_MATCH, C.SIGNED_FAILED_CALLBACK), E.MESSAGE),
        contract(A.REDELIVER_ASSET_NOTIFICATION, L.L2_SINGLE_ORDER_SIDE_EFFECT, "建议重投一条尚未成功的既有资产通知。",
                 (C.PAYMENT_IDENTITY_MATCH, C.UNCONVERGED_ASSET_DELIVERY), E.DELIVERY),
    )
