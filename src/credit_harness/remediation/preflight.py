...
from .catalog import ForbiddenRemediationPolicy
...
class RemediationPreflightService:
    ...
    def preview(self, validated: ValidatedRemediationCandidate) -> RemediationPreflightResult:
        ...
        handler = self.PREFLIGHT_HANDLERS.get(action)
        entry = self.validator.catalog.get(action)
        if not callable(handler) or not callable(self.validator.VALIDATOR_HANDLERS.get(action)):
            status, reasons = S.BLOCKED, [R.ACTION_NOT_ALLOWED]
        elif ForbiddenRemediationPolicy().check(entry):
            status, reasons = S.BLOCKED, list(ForbiddenRemediationPolicy().check(entry))
        elif target.tenant_id != case.tenant_id or target.internal_order_id != case.internal_order_id:
            ...
            # Handlers may suppress already-satisfied actions before the stale
            # check. No handler can grant an intent without shared validation.
            status, reasons, support = handler(self, state, validated)
            if status != S.NOT_NEEDED:
                if validated.snapshot_id != snapshot.snapshot_id:
    ...
    def _preflight_replay(self, state, validated):
        index, target = state.index, validated.target
        ...
        if consumed:
            return S.NOT_NEEDED, [R.ACTION_ALREADY_SATISFIED], support
        ...
        if mismatch:
            ...
            if not deployment:
                return S.BLOCKED, [R.DEPLOYMENT_STATE_UNKNOWN], support
            ...
            if not protocols or actual != {values[C.CONSUMER_LOAN_NO_FIELD_TYPE]}:
                return S.BLOCKED, [R.DEPLOYMENT_INCOMPATIBLE], support
            support = tuple(sorted(set(support) | set(index.refs((*deployment, *protocols)))))
        return S.READY_FOR_FUTURE_AUTHORIZATION, [], support

    def _preflight_redelivery(self, state, validated):
        index, target = state.index, validated.target
        facts = RuleFacts(index)
        ...
        if delivered or converged:
            return S.NOT_NEEDED, [R.ACTION_ALREADY_SATISFIED], validated.supporting_evidence_refs
        return S.READY_FOR_FUTURE_AUTHORIZATION, [], validated.supporting_evidence_refs

    def _preflight_pass_through(self, state, validated):
        # Explicitly registered L0/L1: all action conditions are revalidated by
        # the shared fresh validator. This is not a default for unknown actions.
        return S.READY_FOR_FUTURE_AUTHORIZATION, [], validated.supporting_evidence_refs

    PREFLIGHT_HANDLERS = MappingProxyType({
        A.REPLAY_CALLBACK_CONSUMPTION: _preflight_replay,
        A.REDELIVER_ASSET_NOTIFICATION: _preflight_redelivery,
        A.CREATE_RECONCILIATION_TASK: _preflight_pass_through,
        A.REQUEST_OPERATOR_REVIEW: _preflight_pass_through,
        A.NO_REMEDIATION: _preflight_pass_through,
    })
